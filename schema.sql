...
,
  KEY `suppliers_kategorie` (`kategorie_id`),
...
--
-- Table structure for table `support_meldungen`
--

DROP TABLE IF EXISTS `support_meldungen`;
CREATE TABLE `support_meldungen` (
  ...
  `kontext` text COLLATE utf8mb4_unicode_ci DEFAULT NULL,
...
-- ── v1.3: Banking ─────────────────────────────────────────────────────────
DROP TABLE IF EXISTS `bank_transaktionen`;
DROP TABLE IF EXISTS `bank_importe`;
CREATE TABLE `bank_importe` (
  ...
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `bank_importe_konto` (`bank_account_id`),
...
CREATE TABLE `bank_transaktionen` (
  ...
  KEY `bank_tx_invoice` (`invoice_id`),
  KEY `bank_tx_incoming` (`incoming_invoice_id`),
...
DROP TABLE IF EXISTS `company_kennwerte`;
CREATE TABLE `company_kennwerte` (
  ...
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `company_kennwerte_beleg` (`post_eingang_id`),
