...
import { CalendarDays, Check, Copy, List, RefreshCw , Search } from "lucide-react";
...
export default function Zahlungsziele() {
  ...
  const sort = useSortierung<Eintrag>("datum");
  const gefiltert = ((eintraege.data ?? []) as Eintrag[]).filter(
    (e) => !q.trim() || e.titel.toLowerCase().includes(q.toLowerCase()) || (e.hinweis ?? "").toLowerCase().includes(q.toLowerCase()),
  );
  const zeilenListe = sort.sortiere(gefiltert, (e, key) =>
    key === "datum" ? e.datum : key === "titel" ? e.titel : key === "art" ? e.art
    : key === "betrag" ? (e.betrag !== null ? Number(e.betrag) : null) : null,
  );
        ...
        <>
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-neutral-200 bg-neutral-50 text-left text-xs text-neutral-500">
              <tr>
                <th className="cursor-pointer select-none px-3 py-2 font-medium" onClick={() => sort.umschalten("datum")}>Datum<sort.KopfIcon k="datum" /></th>
                <th className="cursor-pointer select-none px-3 py-2 font-medium" onClick={() => sort.umschalten("titel")}>Eintrag<sort.KopfIcon k="titel" /></th>
                <th className="cursor-pointer select-none px-3 py-2 font-medium" onClick={() => sort.umschalten("art")}>Art<sort.KopfIcon k="art" /></th>
